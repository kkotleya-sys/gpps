...
export function BusInfo({ bus, userLocation, onClose, driverRouteId }: BusInfoProps) {
  const { profile } = useAuth();
  const isDriver = profile?.bus_number === bus.bus_number;
  void userLocation;

  return (
    <BusProfile bus={bus} onClose={onClose} isDriver={isDriver} driverRouteId={driverRouteId} />
  );
}
